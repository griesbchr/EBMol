# Code from https://github.com/ehoogeboom/e3_diffusion_for_molecules/blob/main/qm9/bond_analyze.py
...
# bond analyze ob
def generate_ob_molecule(positions, atom_type, atom_decoder, as_rdkit=False):
    # Setup Open Babel Molecule
    mol = openbabel.OBMol()
    mol.BeginModify()
    ...
    for i in range(n_atoms):
        ...
        element_symbol = atom_decoder[atom_type[i]]
        atomic_num = ATOM_SYMBOL_TO_Z[element_symbol]
        a.SetAtomicNum(atomic_num)
        a.SetVector(float(positions[i][0]), float(positions[i][1]), float(positions[i][2]))

    # Run Perception (Connects atoms and assigns bond orders)
    mol.ConnectTheDots()
...
def build_xae_molecule_openbabel(positions, atom_types, atom_decoder):
    """ 
    ...
    mol = generate_ob_molecule(positions, atom_types, atom_decoder)
