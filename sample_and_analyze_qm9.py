...
from ebmol.ebm_utils import RandomMoleculeGenerator, ShapeEnergy
...
data_path = root + "/data/qm9"


exp_name = "qm9_ls_msp_eregatom"

latest = True
epoch = None
iteration = None

# name of sample file to load for evaluation, should be in samples folder, if None, no samples will be loaded and evaluation will be done on the generated samples
load_samples_name = ""
...
def transform_zero_center(data):
    ...
    return data
filter_n_atoms = None
dataset_test = QM9(data_path, split="test", filter_n_atoms=filter_n_atoms, transform=transform_zero_center)

dataset_test.shuffle()
dataset_train = QM9(data_path, split="train", filter_n_atoms=filter_n_atoms, transform=transform_zero_center)
...
if config.predict_atom_types:
    prior_type = "dirichlet"
    ebm.generate_random.set_atomtype_distribution(torch.tensor([1/config.n_feat]*config.n_feat, dtype=torch.float32), type=prior_type)
else:
    ebm.generate_random.set_atomtype_distribution(dataset_train.x[:,:5].view(-1,config.num_atoms,5).detach().cpu())
...
if load_samples_name is None or load_samples_name == "":
    ...
    # weights for the composite energies, each list entry for one composite, guidance strength for temp=1 (higher temp -> more guidance), no guidance for relaxation
    "composite_weights" : [50],  

    # relaxation sampler parameters
    "relaxation_steps": 50, # QM9: 50, geom: 200/1k, num steps to sample at zero temp                     default: 10
    "rlx_temp": 0.0,
    "rlx_min_cat_x": 0.0,
    "rlx_step_size_pos": 0.01,  #qm9: 0.01, geomv5: 0.01, geom: 0.005
    "rlx_step_size_x": 0.01,    #qm9: 0.01, geomv5: 0.01, geom: 0.005
    "rlx_phi": 0.00,
    ...
    "enable_fresh_noise_transport": False,

    "persistent": True,
    }

    #if isinstance(ebm.sampler, ParallelTemperingSampler):
    #    sampler = ebm.sampler.sampler
    #else:
    #    sampler = ebm.sampler
#
    #sampler.sigma_scale = 0.0
    #sampler.sigma_scale_pos = 0.6   # qm9: 0.6, geom: 0.2,
    #sampler.sigma_scale_cat = 0.6   # qm9: 0.6, geom: 0.4
    #sampler.phi = 0.0
    #sampler.step_size_pos = 0.1        #newqm9: 0.1, newgeom: 0.05, geom: 0.01
    #sampler.step_size_x = 0.1        #newqm9: 0.1, newgeom: 0.05, geom: 0.01
    #sampler.min_cat_x = 0.0015      #newqm9: 0.0015, geom: 0.0005
    ...
    samples_batch, infos = pt_sampler._sample(batch_size=num_samples, verbose=True, samples=None)
...
results = evaluation.compute_stability_batch(samples_batch, atom_decoder=dataset_test.atom_decoder, verbose=True, open_babel=False, debug=False)
_ = evaluation.compute_rdkit_metrics_batch(samples_batch, dataset=dataset_train, verbose=True, open_babel=False, with_conformer=False, limit_bonds_to_one=False)
